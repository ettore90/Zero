...
const SchedulerModal: React.FC<SchedulerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-0 backdrop-blur-sm md:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-labelledby="scheduler-title" className="flex h-full w-full flex-col overflow-hidden border-0 bg-white shadow-2xl transition-colors dark:bg-dark-900 md:h-auto md:max-h-[90vh] md:max-w-2xl md:rounded-xl md:border dark:border-slate-800">
        <div className="p-4 border-b dark:border-slate-800 bg-gray-50 dark:bg-dark-950 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 id="scheduler-title" className="text-lg font-bold text-gray-800 dark:text-white">Message Scheduler</h3>
          </div>
          <button ref={closeButtonRef} type="button" onClick={onClose} aria-label="Close scheduler" className="flex h-10 w-10 items-center justify-center rounded-lg text-gray-500 transition-colors hover:bg-slate-200 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-slate-800 dark:hover:text-gray-200">
        ...
        <div className="flex h-full min-h-0 flex-col overflow-y-auto md:flex-row md:overflow-hidden">
            {/* Create Form */}
            <div className="w-full shrink-0 border-b bg-gray-50/50 p-4 dark:border-slate-800 dark:bg-dark-950/50 md:w-1/3 md:border-b-0 md:border-r md:p-6 md:overflow-y-auto">
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">Create Schedule</h4>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="scheduler-agent" className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Target Agent</label>
                        <select id="scheduler-agent"
                    ...
                    <div>
                        <label htmlFor="scheduler-time" className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Execution Time</label>
                        <input id="scheduler-time"
                            type="datetime-local"
                            value={scheduleTime}
                    ...
                    <div>
                        <label htmlFor="scheduler-message" className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Message</label>
                        <textarea id="scheduler-message"
                            value={content}
                    ...
                    <button type="submit" className="min-h-11 w-full rounded-lg bg-nebula-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition-colors hover:bg-nebula-700">
                        Schedule Message
                    </button>
                </form>
            </div>
