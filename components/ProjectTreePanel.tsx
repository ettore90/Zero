...
// Main ProjectPanel
// ---------------------------------------------------------------------------
const ProjectTreePanel: React.FC<ProjectTreePanelProps> = ({
  ...
  const loadFileTree = useCallback(async (projectPath: string, dirPath: string | null) => {
    ...
    try {
      ...
      if (dirPath !== null) {
        ...
        setLoadedDirs(prev => { const s = new Set(prev); s.add(dirPath); return s; });
      } else {
  ...
  useEffect(() => {
    ...
    setLoadingDirs(new Set());
    setLoadedDirs(new Set());
    void loadFileTree(activeProject.path, null);
    if (activeProject.gitEnabled) void loadGitInfo(activeProject.path);
    else setGitInfo(null);
  }, [activeProject?.path, activeProject?.gitEnabled, loadFileTree, loadGitInfo]);
  ...
  const handleToggleDir = useCallback((path: string) => {
    ...
    setFileTree(prev => {
      ...
      if (!node.expanded && !alreadyLoaded && !isLoading) {
        void loadFileTree(activeProject.path, path);
      }
      return updateNodeExpanded(prev, path, !node.expanded);
    });
  }, [activeProject?.path, loadFileTree, loadedDirs, loadingDirs]);
