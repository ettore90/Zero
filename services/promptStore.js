...
// SQLite binds only numbers, strings, bigints, buffers and null, so JSON booleans
// coming from the HTTP layer must be coerced before they reach a prepared statement.
function sqliteFlag(value, fallback = 0) {
  ...
  if (typeof candidate === 'string') return ['1', 'true', 'yes', 'on'].includes(candidate.trim().toLowerCase()) ? 1 : 0;
  return candidate ? 1 : 0;
}
function sqliteInteger(value, fallback = null) {
...
export function createPromptBlockVersion(blockId, payload = {}) { const nextPayload = { ...payload, metadata: sanitizeBlockMetadata(payload?.metadata || {}) }; const version = upsertBlockVersion(blockId, nextPayload); if (!version) return version; const db = getDb(); const ts = nowSeconds(); // Reancora somente refs explicitamente pinados para a nova versão criada; refs que seguem current permanecem inalterados.
 const refs = db.prepare('SELECT * FROM prompt_document_block_refs WHERE block_id = ?').all(blockId); const tx = db.transaction(() => { for (const row of refs) { const ref = toBlockRef(row); if (!ref || ref.followCurrent !== false || !ref.pinnedBlockVersionId) continue; const nextMetadata = { ...(ref.metadata || {}), pinnedBlockVersionId: version.id, pinned_block_version_id: version.id, followCurrent: false, follow_current: false }; db.prepare('UPDATE prompt_document_block_refs SET block_version_id = ?, metadata = ?, updated_at = ? WHERE id = ?').run(version.id, JSON.stringify(nextMetadata), ts, ref.id); } }); tx(); return version; }
